...
class MesaView(View):
    ...
    def _initialize_mesa(self) -> None:
        ...
        if self.ctx is None:
            raise RuntimeError("Failed to create Mesa context")

        self.buffer: npt.NDArray[np.uint8] = GL.arrays.GLubyteArray.zeros((height, width, 4))  # type: ignore

        # Activate the context with the created buffer
        if not osmesa.OSMesaMakeCurrent(self.ctx, self.buffer, GL.GL_UNSIGNED_BYTE, width, height):
            raise RuntimeError("Failed to activate Mesa context")

        # Check for errors after context activation (optional)
        error = GL.glGetError()
        if error != GL.GL_NO_ERROR:
            print(f"OpenGL error after context activation: {GL.gluErrorString(error)}")
